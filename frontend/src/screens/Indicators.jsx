...
const Indicators = ({ stockStats }) => {
    ...
    const calculateSixMonthReturn = (position) => {
        const currentValue = Object.values(stockStats)[position]
        const prevValue = Object.values(stockStats)[position + 179]
        if(currentValue !== undefined && prevValue !== undefined) {
            const currentClose = parseFloat(Object.values(currentValue)[3])
            const prevClose = parseFloat(Object.values(prevValue)[3])
            return (currentClose - prevClose) / prevClose
    ...
    return (
        <>
        <td> {calculateStockSentiment(1, 23) == 1 ? 'Bearish' : calculateStockSentiment(1, 23) == 0 ? 'Bullish': ''}</td>
        {((calculateDailyBullHigh(1, 5) == calculateDailyBullHigh(1, 20)) && calculateDailyBullHigh(1, 5) > 0 && calculateDailyBullHigh(1, 20) > 0) ? <td>Go Long</td> :
        ((calculateDailyBullLow(1, 5) == calculateDailyBullLow(1, 20)) && calculateDailyBullLow(1, 5) > 0 && calculateDailyBullLow(1, 20) > 0) ? <td>Go Short</td> : <td></td>}
       <td> {calculateStockSentiment(1, 23) == 1 ? `${calculateBearishBuyBottom(1)}-${calculateBearishSellTop(1)}` : `${calculateBullishBuyBottom(1)}-${calculateBullishSellTop(1)}`}</td>
        {calculateRollingEnergy(1, 6, 23) == 1 ? <td>Rising Bull</td> : calculateRollingEnergy(1, 6, 23) == 0 ? <td>Falling Bear</td> : <td></td>}
       
        {calculateStockVolumes(1) ? <td>Volume Spike</td> : <td></td>}
        <td> {calculateStockSentiment(1, 10) == 1 ? 'Bearish' : calculateStockSentiment(1, 10) == 0 ? 'Bullish': ''}</td>
        {calculateVolatilityTrend(1) == 1 ? <td>Volatility Rising</td> : calculateVolatilityTrend(1) == 0 ? <td>Volatility Falling</td> : <td></td>}
        {calculatePriceTrend(1) == 1 ? <td>Price To Fall</td> : calculatePriceTrend(1) == 0 ? <td>Price To Rise</td> : <td></td>}
        </>
    )
}
