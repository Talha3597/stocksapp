...
const Indicators = ({ stockStats, sixMonthData }) => {
    useEffect(() => {
        console.log('stock stats : ', stockStats)
    }, [stockStats, sixMonthData])
        ...
        if(dataLength >= days) {
            ...
            for (let ind = position; ind < (position + days); ind++) {
                ...
                if(currentValue === undefined || previousValue === undefined) {
                    sellingPressure.push(0)
                    buyingPressure.push(0)
                } else {
                    sellingPressure.push(parseFloat(Object.values(currentValue)[2]) - parseFloat(Object.values(previousValue)[3]))
                    buyingPressure.push(parseFloat(Object.values(currentValue)[1]) - parseFloat(Object.values(previousValue)[3]))
                }
            }
    ...
    const calculateOneMonthReturn = (position) => {
        const currentValue = Object.values(stockStats)[position]
        const prevValue = Object.values(stockStats)[position + 29]
        if(currentValue !== undefined && prevValue !== undefined) {
            const currentClose = parseFloat(Object.values(currentValue)[3])
            const prevClose = parseFloat(Object.values(prevValue)[3])
            return ((currentClose - prevClose) / prevClose).toFixed(5)
    ...
    const calculateThreeMonthReturn = (position) => {
        const currentValue = Object.values(stockStats)[position]
        const prevValue = Object.values(stockStats)[position + 89]
        if(currentValue !== undefined && prevValue !== undefined) {
            const currentClose = parseFloat(Object.values(currentValue)[3])
            const prevClose = parseFloat(Object.values(prevValue)[3])
            return ((currentClose - prevClose) / prevClose).toFixed(5)
        ...
        <Table bordered hover responsive>
            <thead>
                <tr>
                    <th>Date</th>
                    <th>Price</th>
                    <th>Bullish Bottom</th>
                    <th>Bullish Top</th>
                    <th>Bearish Bottom</th>
                    <th>Bearish Top</th>
                    <th>Weekly Bull</th>
                    <th>Weekly Bear</th>
                    <th>10 Day Daily Sentiment</th>
                    <th>23 Day Daily Sentiment</th>
                    <th>Volume Spike</th>
                    <th>Rolling Energy</th>
                    <th>Volatility Trend</th>
                    <th>Correlation Trend</th>
                    <th>Price Trend</th>
                    <th>One Day return</th>
                    <th>Five Day return</th>
            ...
            <tbody>
                {Object.keys(stockStats).map((entry, index) => {
                    ...
                    return (
                        <tr>
                            <td>{entry}</td>
                            <td>${stockStats[entry]['4. close']}</td>
                            <td>${calculateBullishBuyBottom(index)}</td>
                            <td>${calculateBullishSellTop(index)}</td>
                            <td>${calculateBearishBuyBottom(index)}</td>
                            <td>${calculateBearishSellTop(index)}</td>
                            {((calculateDailyBullHigh(index, 5) == calculateDailyBullHigh(index, 20)) && calculateDailyBullHigh(index, 5) > 0 && calculateDailyBullHigh(index, 20) > 0) ? <td>Go Long</td> : <td></td>}
                            {((calculateDailyBullLow(index, 5) == calculateDailyBullLow(index, 20)) && calculateDailyBullLow(index, 5) > 0 && calculateDailyBullLow(index, 20) > 0) ? <td>Go Short</td> : <td></td>}
                            {calculateStockSentiment(index, 10) == 1 ? <td>Bearish</td> : calculateStockSentiment(index, 10) == 0 ? <td>Bullish</td>: <td></td>}
                            {calculateStockSentiment(index, 23) == 1 ? <td>Bearish</td> : calculateStockSentiment(index, 23) == 0 ? <td>Bullish</td>: <td></td>}
                            {calculateStockVolumes(index) ? <td>Volume Spike</td> : <td></td>}
                            {calculateRollingEnergy(index, 6, 23) == 1 ? <td>Rising Bull</td> : calculateRollingEnergy(index, 6, 23) == 0 ? <td>Falling Bear</td> : <td></td>}
                            {calculateVolatilityTrend(index) == 1 ? <td>Volatility Rising</td> : calculateVolatilityTrend(index) == 0 ? <td>Volatility Falling</td> : <td></td>}
                            {calculateCorrelation(index) < calculateCorrelation(next) ? <td>Falling Correlation</td> : calculateCorrelation(index) > calculateCorrelation(next) ? <td>Rising Correlation</td> : calculateCorrelation(index) == -1 && <td></td>}
                            {calculatePriceTrend(index) == 1 ? <td>Price To Fall</td> : calculatePriceTrend(index) == 0 ? <td>Price To Rise</td> : <td></td>}
